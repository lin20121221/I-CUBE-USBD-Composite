...
#endif
#if (USBD_USE_CDC_EEM == 1)
  uint8_t USBD_CDC_EEM_DESC[CDC_EEM_CONFIG_DESC_SIZE - 0x09];
#endif
#if (USBD_USE_CDC_NCM == 1)
  uint8_t USBD_CDC_NCM_DESC[CDC_NCM_CONFIG_DESC_SIZE - 0x09];
#endif
...
#endif
#if (USBD_USE_MIDI == 1)
  uint8_t USBD_MIDI_DESC[USB_MIDI_CONFIG_DESC_SIZ - 0x09];
#endif
...
#endif
#if (USBD_USE_CDC_EEM == 1)
  USBD_CDC_EEM.Init(pdev, cfgidx);
#endif
#if (USBD_USE_CDC_NCM == 1)
  USBD_CDC_NCM.Init(pdev, cfgidx);
#endif
#if (USBD_USE_CDC_RNDIS == 1)
...
#endif
#if (USBD_USE_CDC_EEM == 1)
  USBD_CDC_EEM.DeInit(pdev, cfgidx);
#endif
#if (USBD_USE_CDC_NCM == 1)
  USBD_CDC_NCM.DeInit(pdev, cfgidx);
#endif
#if (USBD_USE_CDC_RNDIS == 1)
...
#endif
#if (USBD_USE_CDC_EEM == 1)
  if (LOBYTE(req->wIndex) == CDC_EEM_ITF_NBR)
  {
    return USBD_CDC_EEM.Setup(pdev, req);
  }
#endif
#if (USBD_USE_CDC_NCM == 1)
  if (LOBYTE(req->wIndex) == CDC_NCM_CMD_ITF_NBR || LOBYTE(req->wIndex) == CDC_NCM_COM_ITF_NBR)
  {
    return USBD_CDC_NCM.Setup(pdev, req);
  }
#endif
#if (USBD_USE_CDC_RNDIS == 1)
...
#endif
#if (USBD_USE_MIDI == 1)
  if (LOBYTE(req->wIndex) == MIDI_AC_ITF_NBR || LOBYTE(req->wIndex) == MIDI_MS_ITF_NBR)
  {
...
#endif
#if (USBD_USE_CDC_EEM == 1)
  if (epnum == (CDC_EEM_IN_EP & 0x7F))
  {
    return USBD_CDC_EEM.DataIn(pdev, epnum);
  }
#endif
#if (USBD_USE_CDC_NCM == 1)
  if (epnum == (CDC_NCM_IN_EP & 0x7F) || epnum == (CDC_NCM_CMD_EP & 0x7F))
  {
    return USBD_CDC_NCM.DataIn(pdev, epnum);
  }
#endif
#if (USBD_USE_CDC_RNDIS == 1)
...
#if (USBD_USE_CDC_EEM == 1)
  {
    USBD_CDC_EEM_ItfTypeDef *fops = (USBD_CDC_EEM_ItfTypeDef *)(pdev->pUserData_CDC_EEM);
    if (fops != NULL && fops->Process != NULL) { (void)fops->Process(pdev); }
  }
#endif
#if (USBD_USE_CDC_NCM == 1)
  {
...
static uint8_t USBD_COMPOSITE_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  {
    return USBD_CDC_EEM.DataOut(pdev, epnum);
  }
#endif
#if (USBD_USE_CDC_NCM == 1)
  if (epnum == CDC_NCM_OUT_EP)
  {
    return USBD_CDC_NCM.DataOut(pdev, epnum);
  }
#endif
#if (USBD_USE_CDC_RNDIS == 1)
...
#endif
#if (USBD_USE_CDC_EEM == 1)
    if (index == CDC_EEM_STR_DESC_IDX)
    {
      USBD_GetString((uint8_t *)CDC_EEM_STR_DESC, USBD_StrDesc, length);
    }
#endif
#if (USBD_USE_CDC_NCM == 1)
    if (index == CDC_NCM_STR_DESC_IDX)
    {
      USBD_GetString((uint8_t *)CDC_NCM_STR_DESC, USBD_StrDesc, length);
    }
#endif
#if (USBD_USE_CDC_RNDIS == 1)
...
void USBD_COMPOSITE_Mount_Class(void)
{
...
#if (USBD_USE_CDC_EEM == 1)
  ptr = USBD_CDC_EEM.GetFSConfigDescriptor(&len);
  USBD_Update_CDC_EEM_DESC(ptr,
                           interface_no_track,
                           in_ep_track,
                           out_ep_track,
                           USBD_Track_String_Index);
  memcpy(USBD_COMPOSITE_FSCfgDesc.USBD_CDC_EEM_DESC, ptr + 0x09, len - 0x09);

  ptr = USBD_CDC_EEM.GetHSConfigDescriptor(&len);
  USBD_Update_CDC_EEM_DESC(ptr,
                           interface_no_track,
                           in_ep_track,
                           out_ep_track,
                           USBD_Track_String_Index);
  memcpy(USBD_COMPOSITE_HSCfgDesc.USBD_CDC_EEM_DESC, ptr + 0x09, len - 0x09);
...
#if (USBD_USE_CDC_NCM == 1)
  /* NCM uses: 1 CMD EP (interrupt IN) + 1 bulk IN + 1 bulk OUT, 2 interfaces */
  ptr = USBD_CDC_NCM.GetFSConfigDescriptor(&len);
  USBD_Update_CDC_NCM_DESC(ptr,
                            interface_no_track,
                            interface_no_track + 1,
                            in_ep_track,
                            out_ep_track,
                            in_ep_track + 1,
                            USBD_Track_String_Index);
  memcpy(USBD_COMPOSITE_FSCfgDesc.USBD_CDC_NCM_DESC, ptr + 0x09, len - 0x09);

  ptr = USBD_CDC_NCM.GetHSConfigDescriptor(&len);
  USBD_Update_CDC_NCM_DESC(ptr,
                            interface_no_track,
                            interface_no_track + 1,
                            in_ep_track,
                            ...
                            in_ep_track + 1,
                            USBD_Track_String_Index);
  memcpy(USBD_COMPOSITE_HSCfgDesc.USBD_CDC_NCM_DESC, ptr + 0x09, len - 0x09);

  /* Register interface with class driver */
  /* NOTE: USBD_CDC_NCM_RegisterInterface() is called in usb_device.c,
   * just like all other class RegisterInterface calls. */

  in_ep_track += 2;       /* bulk IN + interrupt IN */
  out_ep_track += 1;      /* bulk OUT */
  interface_no_track += 2; /* control + data interfaces */
  USBD_Track_String_Index += 1;
#endif
  ...
  USBD_Update_MIDI_DESC(ptr, interface_no_track, interface_no_track + 1, in_ep_track, out_ep_track, USBD_Track_String_Index);
