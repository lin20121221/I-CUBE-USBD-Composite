...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  if (pdev->id == DEVICE_HS)
  {
    ...
    HAL_PCDEx_SetTxFiFoInBytes(hpcd_USB_OTG_PTR, (CDC_EEM_IN_EP & 0x7F), 512);
#endif
...
#endif
#if (USBD_USE_MIDI == 1)
    HAL_PCDEx_SetTxFiFoInBytes(hpcd_USB_OTG_PTR, (MIDI_IN_EP & 0x7F), 64);
#endif
...
#endif
#if (USBD_USE_CDC_EEM == 1)
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CDC_EEM_IN_EP,  PCD_SNG_BUF, pma_track);
    pma_track += 64;
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, CDC_EEM_OUT_EP, PCD_SNG_BUF, pma_track);
    pma_track += 64;
#endif
#if (USBD_USE_HID_MOUSE == 1)
...
#endif
#if (USBD_USE_MIDI == 1)
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, MIDI_IN_EP, PCD_SNG_BUF, pma_track);
    pma_track += 64;
    HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, MIDI_OUT_EP, PCD_SNG_BUF, pma_track);
    pma_track += 64;
#endif
#if (USBD_USE_CDC_ACM == 1)
    for (uint8_t i = 0; i < USBD_CDC_ACM_COUNT; i++)
